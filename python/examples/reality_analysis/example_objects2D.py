...
import reality_apis.RDAS.reality_data_analysis_service as RAS
...
def main():
    # This example shows how to submit Reality Analysis job to detect 2D Objects in a set of images.
    # For more information about iTwins, see : https://developer.bentley.com/apis/itwins/overview/

    # Inputs
    itwin_id = ""
    client_id = ""
    images = r"path to your image folder"
    images_name = "Test Moto Photos"
    ...
    service_rda = RAS.RealityDataAnalysisService(token_factory)
    ...
    if not references.has_local_path(photo_context_scene):
        ...
        ret = data_transfer.upload_context_scene(
            photo_context_scene, context_scene_name, itwin_id, references
        )
        if ret.is_error():
            print("Error in upload:", ret.error)
            exit(1)
    ...
    ret = service_rda.create_job(settings, job_name, itwin_id)
    if ret.is_error():
    ...
    ret = data_transfer.download_context_scene(objects2D_id, output_path, itwin_id, references)
    if ret.is_error():
